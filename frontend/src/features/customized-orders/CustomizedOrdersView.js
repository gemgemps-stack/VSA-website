...
const CustomizedOrders = () => {
  ...
  const loadReturnedItems = useCallback(async (customizedOrderId) => {
    if (!customizedOrderId) {
      setReturnedItems([]);
      return;
    }
    try {
      const response = await returnedItemService.getReturnedItemsByCustomizedOrderId(customizedOrderId);
          ...
          >
            <div style={styles.formSection}>
              ...
              {(Number(formData.downPayment || 0) > 0 || requiresModeOfPayment(editingOrder?.status || ORDER_STATUS.FOR_CLIENT_APPROVAL)) && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '10px' }}>
                  ...
                  {/* Row 4: Shop, Order Date and Pickup Date */}
            ...
            >
                <div style={styles.detailsGrid}>
                  ...
                  <div style={styles.detailRow}>
                    <label style={styles.label}>Pickup Date:</label>
                    <span>{selectedOrder?.pickupDate || '-'}</span>
                  </div>
                  <div style={styles.detailRow}>
